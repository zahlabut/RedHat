...
class AnsibleNetworkingFunctionalityTests(unittest.TestCase):
        ...
        for ip in controller_ips:
            ...
            for fil in log_files:
                if fil.endswith('.gz') == True:
                    commands.append("sudo zgrep -i ' ERROR ' /var/log/containers/neutron/"+fil)
                else:
                    commands.append("sudo grep -i ' ERROR ' /var/log/containers/neutron/"+fil)
            for com in commands:
            ...
            ssh_object.ssh_close()
            self.assertNotIn('ERROR', output, 'Failed: ' + ip + ' ERROR detected in server log\n'+output)
    ...
    def test_010_create_bm_guests_in_parallel_and_check_connectivity(self):
        ...
        actual_vlans = get_juniper_sw_get_port_vlan(prms['switch_ip'], prms['switch_user'], prms['switch_password'], prms['baremetal_guest_ports'])
        actual_vlans=[actual_vlans[key] for key in list(actual_vlans.keys())]
    ...
    def test_013_delete_tenant_user(self):
        ...
        new_actual_vlans=[new_actual_vlans[key] for key in list(new_actual_vlans.keys())]
    ...
    def test_016_create_trunk_bm_guest(self):
        ...
        test_pass=False
        for key in list(actual_vlans.keys()):
    ...
    def test_017_no_errors_in_logs(self):
        ...
        for ip in nodes_ips:
            ...
            ssh_object.ssh_close()
        test_failed=False
        for ip in list(actual_errors.keys()):
